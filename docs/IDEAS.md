...
## 7) Next Sprint (Recommended Scope)

1. **Route decomposition kickoff:** extract auth + proposal routes into blueprints.
2. **Startup orchestration:** implement unified startup sequence with severity-based exception policy.
3. **API consistency baseline:** apply standard error envelope and begin contract tests for core endpoints.

This scope is intentionally narrow to maximize compounding value in subsequent sprints.
