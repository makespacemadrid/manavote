...
class TestCalendarBudgetData(unittest.TestCase):
    ...
    def test_calendar_legend_excludes_rejected_label(self):
        """Activity Calendar legend should not include Rejected badge"""
        template = Path("templates/calendar.html").read_text(encoding="utf-8")
        self.assertNotIn("background: #e94560; border-radius: 3px;", template)
