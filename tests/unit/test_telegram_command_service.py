...
def test_vote_command_rejects_malformed_text(tmp_path):
    db_path = _make_db(tmp_path)
    ok, reason = telegram_command_service.process_telegram_vote_command(
        lambda: _connect(db_path), _settings(), lambda message: None, _logger, "alice", "/vote"
    )
...
def test_vote_command_rejects_unknown_member_without_telegram_user_id(tmp_path):
    ...
    ok, reason = telegram_command_service.process_telegram_vote_command(
        lambda: _connect(db_path), _settings(), lambda message: None, _logger, "stranger", "/vote 1", telegram_user_id=None
    )
...
def test_vote_command_allows_unlinked_telegram_user_when_not_required(tmp_path):
    ...
    ok, reason = telegram_command_service.process_telegram_vote_command(
        lambda: _connect(db_path), _settings(), lambda message: None, _logger, "stranger", "/vote 1", telegram_user_id=999
    )
...
def test_proposal_vote_command_rejected_when_record_returns_false(tmp_path):
    ...
    ok, reason = telegram_command_service.process_telegram_proposal_vote_command(
        lambda: _connect(db_path), _settings(), lambda *a, **kw: False, _logger, "alice", "/pvote 7 yes", telegram_user_id=42
    )
    assert (ok, reason) == (False, "telegram_disabled")
...
def test_proposal_vote_command_succeeds_and_forwards_source_telegram(tmp_path):
    ...
    ok, reason = telegram_command_service.process_telegram_proposal_vote_command(
        lambda: _connect(db_path), _settings(), record_proposal_vote, _logger, "alice", "/pvote 7 yes", telegram_user_id=42
    )
...
def test_proposal_vote_command_rejects_closed_proposal(tmp_path):
    ...
    ok, reason = telegram_command_service.process_telegram_proposal_vote_command(
        lambda: _connect(db_path), _settings(), lambda *a, **kw: True, _logger, "alice", "/pvote 7 yes", telegram_user_id=42
    )
    assert (ok, reason) == (False, "proposal_closed")


def test_proposal_vote_command_rejects_link_required(tmp_path):
    db_path = _make_db(tmp_path)
    conn = _connect(db_path)
    conn.execute("INSERT INTO proposals (id, status) VALUES (7, 'active')")
    conn.commit()
...
def test_vote_callback_rejects_unrecognized_format(tmp_path):
    db_path = _make_db(tmp_path)
    ok, reason = telegram_command_service.process_telegram_vote_callback(
        lambda: _connect(db_path), _settings(), lambda message: None, lambda *a, **kw: True, _logger, "alice", "bogus:1"
    )
    assert (ok, reason) == (False, "invalid_format")


def test_vote_command_disabled_channel_is_audited(tmp_path, caplog):
    db_path = _make_db(tmp_path)

    with caplog.at_level("INFO", logger="test"):
        telegram_command_service.process_telegram_vote_command(
            lambda: _connect(db_path),
            _settings({"poll_vote_mode": "web_only"}),
            lambda message: None,
            _logger,
    ...
    assert any(
        "event=poll_vote_rejected source=telegram" in record.message and "reason_code=channel_disabled" in record.message
        for record in caplog.records
    )


def test_vote_command_link_required_is_audited(tmp_path, caplog):
    db_path = _make_db(tmp_path)
    conn = _connect(db_path)
    conn.execute("INSERT INTO polls (question, options_json, status) VALUES ('Snacks?', '[\"Yes\",\"No\"]', 'open')")
    conn.commit()
    ...
    assert any(
        "event=poll_vote_rejected source=telegram" in record.message and "reason_code=link_required" in record.message
        for record in caplog.records
    )


def test_proposal_vote_command_link_required_is_audited(tmp_path, caplog):
    db_path = _make_db(tmp_path)
    conn = _connect(db_path)
    conn.execute("INSERT INTO proposals (id, status) VALUES (7, 'active')")
    conn.commit()
    ...
    with caplog.at_level("INFO", logger="test"):
        telegram_command_service.process_telegram_proposal_vote_command(
            lambda: _connect(db_path),
            _settings({"telegram_require_linked_vote": "true"}),
            lambda *a, **kw: True,
            _logger,
    ...
    assert any(
        "event=proposal_vote_rejected source=telegram" in record.message and "reason_code=link_required" in record.message
        for record in caplog.records
    )
