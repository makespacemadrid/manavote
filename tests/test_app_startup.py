...
class TestAppStartup(unittest.TestCase):
    ...
    def test_run_startup_steps_executes_in_order(self):
        events = []
        with patch("app.startup.ensure_db_ready", side_effect=lambda: events.append("db")), \
             patch("app.startup.check_auto_backup", side_effect=lambda *_: events.append("backup")), \
             patch("app.startup.start_scheduler", side_effect=lambda *_: events.append("scheduler")):
    ...
    def test_scheduler_failure_is_warning_not_fatal(self):
        with patch("app.startup.ensure_db_ready"), \
             patch("app.startup.check_auto_backup"), \
             patch("app.startup.start_scheduler", side_effect=OSError("nope")), \
             patch("app.logging.warning") as warning_mock:
            run_startup_steps(app.flask_app, "test.db", "uploads")
    ...
    def test_auto_backup_failure_is_warning_not_fatal(self):
        with patch("app.startup.ensure_db_ready"), \
             ...
             patch("app.startup.check_auto_backup", side_effect=ValueError("bad backup state")), \
             patch("app.logging.warning") as warning_mock:
            run_startup_steps(app.flask_app, "test.db", "uploads")
        warning_mock.assert_called()

    def test_create_app_logs_warning_on_optional_import_error(self):
        with patch("app.run_startup_steps", side_effect=ImportError("optional dep missing")), \
             patch("app.logging.warning") as warning_mock:
    ...
    def test_test_env_skips_scheduler_and_auto_backup(self):
        with patch("app.startup.ensure_db_ready"), \
             patch("app.startup.start_scheduler") as scheduler_mock, \
             patch("app.startup.check_auto_backup") as backup_mock:
