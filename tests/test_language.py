...
class TestProposalsFiltersAndStatus(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        budget_app.app.config["TESTING"] = True
        budget_app.app.config["WTF_CSRF_ENABLED"] = False
    ...
    def test_proposals_filter_buttons_title_case(self):
        """Proposals filter buttons show Title Case"""
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
    ...
    def test_proposals_status_tags_lowercase(self):
        """Proposals proposal status tags show lowercase"""
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
        self.assertIn("status-active", html)

    def test_proposals_spanish_filter_buttons_title_case(self):
        """Spanish proposals filter buttons show Title Case"""
        self.client.get("/set-language/es")
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
...
class TestProposalsFilters(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        budget_app.app.config["TESTING"] = True
        budget_app.app.config["WTF_CSRF_ENABLED"] = False
    ...
    def test_proposals_shows_all_filter(self):
        """Proposals shows All filter button"""
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
        self.assertIn("All", html)

    def test_proposals_shows_active_filter(self):
        """Proposals shows Active filter button"""
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
        self.assertIn("Active", html)

    def test_proposals_shows_approved_filter(self):
        """Proposals shows Approved filter button"""
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
        self.assertIn("Approved", html)

    def test_proposals_shows_pending_budget_filter(self):
        """Proposals shows Pending Budget filter button"""
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
        self.assertIn("Pending Budget", html)

    def test_proposals_shows_purchased_filter(self):
        """Proposals shows Purchased filter button"""
        response = self.client.get("/proposals")
        self.assertEqual(response.status_code, 200)
        html = response.data.decode("utf-8")
        self.assertIn("Purchased", html)

    def test_proposals_filter_case_title(self):
