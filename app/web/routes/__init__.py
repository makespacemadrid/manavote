...
def register_blueprints(app):
    ...
    for legacy_name, blueprint_name in legacy_endpoint_aliases.items():
        if blueprint_name not in app.view_functions:
            continue

        # Keep backward compatibility for direct lookups in app.view_functions.
        if legacy_name not in app.view_functions:
            app.view_functions[legacy_name] = app.view_functions[blueprint_name]

        # Also register URL map aliases so url_for("legacy_name") resolves.
        if legacy_name in rules_by_endpoint:
            continue
        for rule in rules_by_endpoint.get(blueprint_name, []):
            methods = sorted(m for m in (rule.methods or set()) if m not in {"HEAD", "OPTIONS"})
            app.add_url_rule(
                rule.rule,
                endpoint=legacy_name,
                view_func=app.view_functions[blueprint_name],
